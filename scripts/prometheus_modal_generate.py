...
class Prometheus7B_8bit:
    def __enter__(self):
        ...
        config = PeftConfig.from_pretrained(str(VOL_MOUNT_PATH / "prometheus-7b_SFT_0_completions_only" / "final_checkpoint"))
        
        model = PeftModel.from_pretrained(model, str(VOL_MOUNT_PATH / "prometheus-7b_SFT_0_completions_only" / "final_checkpoint"))
    ...
    def train_dpo(self, dataset):
        ...
        dpo_trainer = DPOTrainer(self.model, self.model_ref, args=training_args, beta=0.4, train_dataset=train_dataset, tokenizer=self.tokenizer, peft_config=peft_config)
        dpo_trainer.train()
        dpo_trainer.save_model(str(VOL_MOUNT_PATH / "model"))

        dpo_trainer.model.save_pretrained(str(VOL_MOUNT_PATH / "model" / "final_checkpoint"))
        output_vol.commit()
        print("✅ done")
    ...
    def generate(self, prompt: str):
        ...
        outputs = self.model.generate(input_ids=input_ids, temperature=1.0, top_p=0.4, max_new_tokens=512, repetition_penalty=1.03)
...
TEMP_PATH = "/mnt/c/Users/amangokrani/OneDrive - Microsoft/Personal/evaluations/outputs/temp/prometheus-7b_SFT_0_completions_only/valid"

@stub.local_entrypoint()
def main():
    dataset = load_dataset("json", data_files="/mnt/c/Users/amangokrani/OneDrive - Microsoft/Personal/evaluations/outputs/valid.gpt-4-1106-preview.pred.fullnote.json")
