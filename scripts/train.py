...
def main(model_args, data_args, training_args, finetuning_args, train_dataset, eval_dataset):
    model, model_ref, peft_config, tokenizer = create_and_prepare_model.local(model_args, finetuning_args)
    ...
    trainer.save_model(str(Path("/finetune") / training_args.output_dir))
    trainer.model.save_pretrained(str(Path("/finetune") / training_args.output_dir / "final_checkpoint"))
...
if __name__ == "__main__":
    ...
    with stub.run():
        main.remote(model_args, data_args, training_args, finetuning_args, train_dataset=datasets["train"].to_list(), eval_dataset=datasets["test"].to_list() if "test" in datasets.keys() else None)